...
# -------------------------------------------------------------------------
val_plot_data_by_set <- purrr::map2_dfr(lm_models_log, names(lm_models_log), function(model, fml) {
...
invisible(iwalk(plots_by_method, function(p, mth) {
  ggsave(paste0("figures/external-validation/validation_png-log/obs_pred_external_", mth, ".png"),  p, width = 8, height = 6, dpi = 300)
  ggsave(paste0("figures/external-validation/validation_tiff-log/obs_pred_external_", mth, ".tiff"), p, width = 8, height = 6, dpi = 300, compression = "lzw")
}))
...
val_perf_table_by_set <- purrr::map2_dfr(
  lm_models_log, names(lm_models_log),
  function(model, fml) {
    response <- stringr::str_extract(fml, "^[^~]+") %>% stringr::str_trim()
...
print(doc, target = "docx/external-validation/performance_validation_by_set-log.docx")