...
plot_data <- map2_dfr(lm_models_log, names(lm_models_log), function(model, fml) {
  response <- str_extract(fml, "^[^~]+") %>% str_trim()
...
ggsave("figures/model-performance/testing_set_one-log.tiff", obs_pred, width = 8, height = 8)
ggsave("figures/model-performance/testing_set_one-log.png", obs_pred, width = 8, height = 8)
...
print(doc, target = "docx/model-performance/performance_set1-log.docx")
