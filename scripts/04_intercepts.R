...
# Intercepts linear models using set one
...
fit_lm <- function(response, predictor, train_data, test_data, intercept = TRUE) {
  ...
  ss_res <- sum((truth - preds)^2, na.rm = TRUE)
  ss_tot <- if (intercept) sum((truth - mean(truth, na.rm = TRUE))^2, na.rm = TRUE) else sum(truth^2, na.rm = TRUE)
  r2_manual <- 1 - ss_res / ss_tot
  
  # pull slope row safely
  tidy <- broom::tidy(mod)
  slope_row <- tidy[tidy$term == predictor, , drop = FALSE]
  ...
  tibble::tibble(
    ...
    intercept  = intercept,
    r_squared  = r2_manual,
    sigma      = stats::sd(truth - preds, na.rm = TRUE),
    p_value    = slope_row$p.value[1],
    slope      = slope_row$estimate[1],     
    std_error  = slope_row$std.error[1],
...
intercepts <- ggplot(pred_results, aes(x = truth, y = preds, color = intercept)) +
...
ggsave("figures/intercepts/fig_intercepts.tiff", intercepts, width = 8, height = 8)
ggsave("figures/intercepts/fig_intercepts.png", intercepts, width = 8, height = 8)
...
table_export <- model_results %>%
  ...
  select(Response = response, Predictor = predictor, Intercept,
         `Slope` = slope, `Std.Error` = std_error,
         `R²` = r_squared, 
...
print(doc, target = "docx/intercepts/models-table-intercept.docx")
