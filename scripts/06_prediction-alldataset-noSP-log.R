...
obs_pred <- ggplot(plot_data, aes(x = .pred, y = truth)) +
  ...
  labs(
    x = "Predicted",
    y = "Observed",
    title = "Observed vs. Predicted (Test Set) without SP",
    subtitle = "Linear models using log-methods + optional covariates all datasets"
  ) +
  coord_obs_pred() +
...
ggsave("figures/obs_pred_log-noSP.tiff", obs_pred, width = 12, height = 8)
ggsave("figures/obs_pred_log-noSP.png", obs_pred, width = 12, height = 8)
...
save_as_docx(
  coef_flex,
  path = "docx/coef-table-log-noSP.docx"
)
